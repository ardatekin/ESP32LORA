...
void Relay::Init()
{
    ...
    gpio_set_level((gpio_num_t)RELAY_PIN, 0); // Ensure relay is off initially
...
void Relay::Open()
{
    ...
    gpio_set_level((gpio_num_t)RELAY_PIN, 1); // Turn relay on
}

void Relay::Close()
{
    printf("Turning Relay OFF (GPIO %d).\n", RELAY_PIN);
    gpio_set_level((gpio_num_t)RELAY_PIN, 0); // Turn relay off
}
